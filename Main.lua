...
local function toggleNoclip(s)
	...
	if s then
		...
		local char=LocalPlayer.Character if not char then return end
		for _,p in ipairs(char:GetDescendants())do
			if p:IsA("BasePart")then p.CanCollide=true end end
	end
end
...
TB.Position=UDim2.new(0,16,0.5,-25)
TB.BackgroundColor3=Color3.fromRGB(220,50,50)
...
Instance.new("UICorner",TB).CornerRadius=UDim.new(0,25)
...
ms.Color=Color3.fromRGB(220,50,50) ms.Thickness=2
local vis=true
TB.MouseButton1Click:Connect(function()
	if td then return end
	vis=not vis Main.Visible=vis
	TB.BackgroundColor3=vis and Color3.fromRGB(220,50,50)or Color3.fromRGB(50,50,50)end)
...
for i,name in ipairs(tabNames)do
	...
	leftBar.BackgroundColor3=i==1 and Color3.fromRGB(255,255,255)or Color3.fromRGB(180,35,35)
	leftBar.BorderSizePixel=0 leftBar.Parent=TB2
	tabBtns[i]={btn=TB2,bar=leftBar}
...
local function makeToggle(parent,label,order,callback)
	...
	local function flip()
		on=not on
		TweenService:Create(Tog,TweenInfo.new(0.15),{BackgroundColor3=on and Color3.fromRGB(255,255,255)or Color3.fromRGB(100,20,20)}):Play()
		TweenService:Create(Knob,TweenInfo.new(0.15),{
			BackgroundColor3=on and Color3.fromRGB(220,50,50)or Color3.fromRGB(255,255,255),
			Position=on and UDim2.new(0,23,0.5,-9)or UDim2.new(0,3,0.5,-9)}):Play()
		callback(on)
	end
...
local function makeSlider(parent,label,order,mn,mx,def,callback)
	...
	local function upd(x)
		...
		SK.Position=UDim2.new(rel,-10,0.5,-10) callback(val) end
...
makeToggle(VI,"Full Bright",2,function(on)
	if on then game:GetService("Lighting").Brightness=10
	else game:GetService("Lighting").Brightness=2 end end)
makeToggle(VI,"No Fog",3,function(on)
	if on then game:GetService("Lighting").FogEnd=1e6
	else game:GetService("Lighting").FogEnd=100000 end end)
local MI=panels[9]
...
DPad.BackgroundTransparency=1 DPad.Visible=false DPad.ZIndex=9 DPad.Parent=SG
local function flyBtn(parent,txt,x,y,w,h,key)
	local b=Instance.new("TextButton")
	b.Size=UDim2.new(0,w,0,h) b.Position=UDim2.new(0,x,0,y)
	b.BackgroundColor3=Color3.fromRGB(180,35,35) b.BorderSizePixel=0
	b.Text=txt b.TextColor3=Color3.fromRGB(255,255,255)
	b.TextSize=18 b.Font=Enum.Font.GothamBold b.ZIndex=10 b.Parent=parent
	Instance.new("UICorner",b).CornerRadius=UDim.new(0,8)
	b.InputBegan:Connect(function(i)if i.UserInputType==Enum.UserInputType.Touch then mi[key]=true end end)
	b.InputEnded:Connect(function(i)if i.UserInputType==Enum.UserInputType.Touch then mi[key]=false end end)
end
...
VP.BackgroundTransparency=1 VP.Visible=false VP.ZIndex=9 VP.Parent=SG
